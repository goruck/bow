...
/* compile with
gcc -Wall manual_classifier.cpp -I/usr/local/include/ \
    -L/usr/lib/ -lstdc++ \
...
int main(int argc, char * const argv[]) {
	...
	namedWindow("pic");
	setMouseCallback( "pic", onMouse, NULL );
		...
		while (true) {
			...
			if (c == ' ') {
				...
				ofs << " " << selection.x << "," << selection.y << "," << selection.width << "," << selection.height;
                                rectangle(img, selection, Scalar(0,255,0), 2, 8, 0);
				ofs << " " << char_to_class(c);
